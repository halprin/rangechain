		...
		for innerValue == nil {
			if currentGenerator == nil {
				...
				if helper.IsSlice(currentValue) {
					...
					channelCurrentValue := currentValue.(chan interface{})
					currentGenerator = generator.FromChannel(channelCurrentValue)
				} else if helper.IsMap(currentValue) {
					mapCurrentValue := helper.InterfaceMap(currentValue)
					currentGenerator = generator.FromMap(mapCurrentValue)
				} else {
